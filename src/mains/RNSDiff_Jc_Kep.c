 ...
 *
 * This program computes rotating equilibrium configurations of neutron
 * stars containing both baryonic and dark matter fluids, under general
 * relativistic hydrostationary equilibrium conditions with differential
 * rotation.
 *
 * Unlike the basic sequence construction, this version targets models
 * with fixed total angular momentum (J_tot) and dark matter angular
 * velocity (Omega_DM). It iteratively adjusts the baryonic and dark matter
 * axis ratios and the dark matter central energy density to simultaneously
 * satisfy constraints on the global dark matter fraction and angular velocity.
 *
 * The code reads parameters from a configuration file and supports
 * warm-starting from existing HDF5 output to improve convergence.
 *
 * Output includes global quantities (masses, radii, angular momenta,
 * rotational frequencies, virial identities) and optionally 2D slices
 * of metric and matter fields stored in HDF5 format.
 *
 *
 **************************************************************************/
#define _GNU_SOURCE
...
int main(int argc, char **argv) {
    ...
    double J_BM, J_DM, JBM_Target, Jtot_Target, Omega_DM_target, fJ_DM;
    double kep_fac = -1;
    ...
    bool skip = false;
    int c;
    for (c = getopt(argc, argv, "+hc:j:d:k:");
         c != EOF;
         c = getopt(argc, argv, "+hc:j:d:k:")) {
        switch (c) {
            case 'c':
                ...
                break;
            case 'j':
                sscanf(optarg, "%lf", &Jtot_Target);
                break;
            case 'd':
                sscanf(optarg, "%lf", &Omega_DM_target);
                break;
            case 'k':
                sscanf(optarg, "%lf", &kep_fac);
                break;
            case 'h':
                printf("\n");
                printf("Help:\n");
                ...
                printf("\t>> ./RNS\n\n");
                printf("Run \n\t>> ./RNS -c config.d [-j Jtot_Target] [-d Omega_DM_target] [-k kep_fac]\n");
                printf("to start the computations.\nThe flag -k is a float between 0 and 1: if one, finds the DM at the kepler frequency, else it looks for Omega_DM = kep_fac * Omega_DM_kepler. If both -d and -k are specified, -k has priority\n");
                return 0;
            default:
    ...
    // Task: find ec_DM and r_ratio_BM that give fdm and Jtot for a fixed ec_BM and r_ratio_DM
    ...
    while (Jiter < MAXIT) {
        ...
        // Loop to change r_ratio_DM and reach Omega_kepler
        int Odm_iter = 0;
        r_ratio_step_DM = r_ratio_step;

        double r_ratio_DM_lo = 0.0, r_ratio_DM_hi = 1.0;
        double Omega_rlo_DM = INFINITY, Omega_rhi_DM = 0.0;
        bool accepted_dm_last_step = false;
        while (Odm_iter < MAXIT) {
            ...
            while (iter < MAXIT) {
                ...
                // If looking for a fraction of the kepler frequency, update it every time
                // Otherwise it remains the fixed value set via CLI
                if (use_fac) {
                    Omega_DM_target = kep_fac * Omega_K_DM;
                }
                ...
                DPRINT(" Computed fdm = %.6f (old = %.6f), Omega_DM = %.6f, Jtot = %.6f\n", fdm, fdm_old, tmp_OmegaDM, Jtot);
            ...
            DPRINT("Post f_DM inner loop: Omega_DM = %.6f, Omega_DM_target = %.6f\n", tmp_OmegaDM, Omega_DM_target);

            if (zero || Omega_DM_target == 0 || fabs(tmp_OmegaDM - Omega_DM_target) / (Omega_DM_target) <= fdm_tol) {
                DPRINT("Omega_DM converged.\n");
                break;
            }

            // Check if Omega_DM is below Omega_DM_target
            if (tmp_OmegaDM <= Omega_DM_target) {
                // Accept and update upper bound
                r_ratio_DM_hi = tmp_rratioDM;
                Omega_rhi_DM = tmp_OmegaDM;

                DPRINT("r_ratio_DM_lo = %g Omega_rlo_DM = %g  r_ratio_DM_hi = %g  Omega_rhi_DM = %g\n", r_ratio_DM_lo, Omega_rlo_DM, r_ratio_DM_hi, Omega_rhi_DM);
                if (r_ratio_DM_lo != 0.0 && fabs(Omega_rhi_DM - Omega_rlo_DM) > 1e-12) {
                    double new_rr = r_ratio_DM_hi - (Omega_rhi_DM - Omega_DM_target) * (r_ratio_DM_hi - r_ratio_DM_lo) / (Omega_rhi_DM - Omega_rlo_DM);
                ...
                if (r_ratio_DM_hi != 1 && fabs(Omega_rlo_DM - Omega_rhi_DM) > 1e-12) {
                    double new_rr = r_ratio_DM_hi - (Omega_rhi_DM - Omega_DM_target) * (r_ratio_DM_hi - r_ratio_DM_lo) / (Omega_rhi_DM - Omega_rlo_DM);
                    tmp_rratioDM = fmax(fmin(new_rr, r_ratio_DM_hi), r_ratio_DM_lo);
        ...
        // Check convergence
        if (Jtot == 0 || fabs(Jtot - Jtot_Target) / Jtot_Target <= fdm_tol) {
